...
# --- NEW: Import specific types for more detailed error handling ---
...
def get_story_from_ai(user_prompt):
    ...
    try:
        # Initialize the Gemini model (Keeping your specified model)
...
A mese végén adj tanácsot a szülőnek, milyen kérdésekkel, hogyan beszélgessen a gyermekeivel a meséről, hogy a meseterápia a lehető leghasznosabb legyen.
        ...
        # --- NEW: Added a try/except block around the loop to catch errors during streaming ---
        try:
            ...
            # --- NEW: This will print the specific error during streaming to your console ---
            print(f"CONSOLE LOG: Hiba a streaming során: {e}")
            yield f"Hiba történt a mese generálása közben. ({e})"

        # --- NEW: Check if the prompt or response was blocked for any reason after the stream finishes ---
        try:
            if response.prompt_feedback.block_reason:
                print(f"CONSOLE LOG: A generálást blokkolta: {response.prompt_feedback.block_reason.name}")
                yield f"\n\nA mese generálását egy szűrő leállította ({response.prompt_feedback.block_reason.name}). Próbálkozzon újra."
        except Exception as e:
             # This handles cases where the 'response' object itself might be invalid after a failed stream
            print(f"CONSOLE LOG: Nem sikerült lekérni a prompt visszajelzését: {e}")


    except Exception as e:
        # This is the outer error handler, now with a console log
        print(f"CONSOLE LOG: Általános hiba a történet generálása során: {str(e)}")
...
def generate_tale():
    ...
    try:
        ...
        # --- NEW: Added a console log to this route's error handler ---
        print(f"CONSOLE LOG: Hiba a /generate_tale útvonalon: {str(e)}")
